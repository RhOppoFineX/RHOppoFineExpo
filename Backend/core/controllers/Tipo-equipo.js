...
const api = '../../RHOppoFineExpo/Backend/core/api/Tipo-equipo.php?action=';
...
function fillTable(filas)
{
   ...
   filas.forEach(fila => {
        ...
        contenido+= `
            <tr>
                ...
                <td><a class="btn btn-warning btn-sm" onclick="actualizarModal(${fila.Id_tipo_equipo})">Modificar</a></td>
				<td><a class="btn btn-danger btn-sm" onclick="confirmDelete('${api}', ${fila.Id_tipo_equipo}, null, 'delete')">Eliminar</a></td>
            </tr>       
        `;//invertidas
   ...
   $('#tabla-tipo-equipo').html(contenido); 
...
{
    $('#Insertar-tipoEquipo')[0].reset();//Id del formulario
    $('#tipoEquipo-insertar').modal('show');//Id del modal
}
...
{   //Id_religion es el parametro para la consulta
    ...
    .done(function(response){
        ...
        if (isJSONString(response))
        {
            ...
            {   //dataset es el resultado de la consulta que devuelve la API
                //Este resultado es un array con los datos
                $('#Id_tipo_equipo').val(resultado.dataset.Id_tipo_equipo);                       
                $('#Tipo_equipo').val(resultado.dataset.Tipo_equipo);//id de cada input
                //en caso de que alla más input ponen sus respectivos id            
                $('#tipo-modificar').modal('show');//id del modal modificar
            }else{
...
$('#modificar-tipo').submit(function()
{
    event.preventDefault();
    $.ajax({
        url: api + 'update',
        type: 'post',
        data: $('#modificar-tipo').serialize(),
        datatype: 'json'
    })
    .done(function(response){
        // Se verifica si la respuesta de la API es una cadena JSON, sino se muestra el resultado en consola
        if (isJSONString(response)) {
            ...
            if (resultado.status) {
                $('#tipo-modificar').modal('hide');//Id del modal modificar
                showTable();
...
$('#Insertar-tipoEquipo').submit(function()
{   
    event.preventDefault();
    $.ajax({
        url: api + 'create',
        type: 'post',
        data: $('#Insertar-tipoEquipo').serialize(),
        datatype: 'json'
    })
    .done(function(response){
        // Se verifica si la respuesta de la API es una cadena JSON, sino se muestra el resultado en consola
        if (isJSONString(response)) {
            ...
            // Se comprueba si el resultado es satisfactorio, sino se muestra la excepción
            if (resultado.status) {
                $('#Insertar-tipoEquipo')[0].reset();//Id del formulario insertar
                $('#tipoEquipo-insertar').modal('hide');//Id del modal insertar
                showTable();
                sweetAlert(1, resultado.message, null);
            } else {
                sweetAlert(2, resultado.exception, null);
