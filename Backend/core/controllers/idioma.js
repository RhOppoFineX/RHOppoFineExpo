...
const api = '../../RHOppoFineExpo/Backend/core/api/idioma.php?action=';
   ...
   filas.forEach(fila => {
        ...
        contenido+= `
            <tr>
                ...
                <td>${fila.idioma}</td>							
                <td><a class="btn btn-warning btn-sm" onclick="actualizarModal(${fila.Id_idioma})">Modificar</a></td>
				<td><a class="btn btn-danger btn-sm" onclick="confirmDelete('${api}', ${fila.Id_idioma}, null)">Deshabilitar</a></td>
            </tr>       
        `;//invertidas
...
{
    $('#insertaridioma')[0].reset();//Id del formulario
    $('#idiomaInsertar').modal('show');//Id del modal
}
...
{   //Id_idioma es el parametro para la consulta
    ...
    .done(function(response){
        ...
        if (isJSONString(response))
        {
            ...
            {   //dataset es el resultado de la consulta que devuelve la API
                //Este resultado es un array con los datos
                $('#id_idioma').val(resultado.dataset.Id_idioma);                       
                $('#idioma').val(resultado.dataset.idioma);//id de cada input
                //en caso de que alla más input ponen sus respectivos id            
                $('#idiomaModificar').modal('show');//id del modal modificar
            }else{
...
$('#actualizaridioma').submit(function()
{
    event.preventDefault();
    $.ajax({
        url: api + 'update',
        type: 'post',
        data: $('#actualizaridioma').serialize(),
        datatype: 'json'
    })
    .done(function(response){
        // Se verifica si la respuesta de la API es una cadena JSON, sino se muestra el resultado en consola
        if (isJSONString(response)) {
            ...
            if (resultado.status) {
                $('#idiomaModificar').modal('hide');//Id del modal modificar
                showTable();
...
$('#insertaridioma').submit(function()
{   
    event.preventDefault();
    $.ajax({
        url: api + 'create',
        type: 'post',
        data: $('#insertaridioma').serialize(),
        datatype: 'json'
    })
    .done(function(response){
        // Se verifica si la respuesta de la API es una cadena JSON, sino se muestra el resultado en consola
        if (isJSONString(response)) {
            ...
            // Se comprueba si el resultado es satisfactorio, sino se muestra la excepción
            if (resultado.status) {
                $('#insertaridioma')[0].reset();//Id del formulario insertar
                $('#idiomaInsertar').modal('hide');//Id del modal insertar
                showTable();
                sweetAlert(1, resultado.message, null);
            } else {
                sweetAlert(2, resultado.exception, null);
